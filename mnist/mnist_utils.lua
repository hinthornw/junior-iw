...
local sum = torch.Tensor(10):zero()
local mean = torch.Tensor(10, 32, 32):zero()
mnist.means = torch.Tensor(10, 32, 32):zero()
for i = 1, (#mnist.train_inputs)[1] do
	lb = mnist.train_outputs [i]
	img = mnist.train_inputs[i]
	-- print("Output = ", lb)
	mean[lb] = mean[lb] + img
	sum[lb] = sum[lb] + 1
end
